...
export const magicRoutes = new Elysia({ prefix: '/magics' }).onBeforeHandle((ctx: any) =>
	...
	.post('/', async ({ body, set }: { body: any; set: any }) => {
		...
		const existing = await db.collection('magics').findOne({ id: body.id });
		if (existing) {
			set.status = 409;
			return { error: 'Magic already exists' };
		}
		await db.collection('magics').insertOne(body);
		set.status = 201;
		return body;
	})
	.put('/:id', async ({ params: { id }, body, set }: { params: { id: string }; body: any; set: any }) => {
		const db = getDb();
		const result = await db.collection('magics').replaceOne({ id }, body);
	...
	.delete('/:id', async ({ params: { id }, set }: { params: { id: string }; set: any }) => {
		const db = getDb();
		const result = await db.collection('magics').deleteOne({ id });
