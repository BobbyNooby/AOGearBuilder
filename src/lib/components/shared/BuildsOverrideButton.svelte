<script>
	import { currentShipBuildId } from '$lib/utils/shipStatsStore';
	import { playCorrect } from '$lib/utils/sound';
	import { currentBuildId, loadCode } from '$lib/utils/statsStore';
	...
	function overrideBuild(savedBuild) {
		...
		if (type == 'gear') {
			savedBuilds = savedBuilds.map((build) =>
				build.name === savedBuild['name'] ? { ...build, code: $currentBuildId } : build
			);
		} else if (type == 'ship') {
			savedBuilds = savedBuilds.map((build) =>
