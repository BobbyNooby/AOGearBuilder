	...
	{#if ready}
		{#if !$isMobile}
			<footer
				...
				class=" opacity-100 transition-opacity duration-300 ease-in-out w-full z-0"
			...
			>
				<nav class="z-0">
					<ul class="flex space-x-1 md:space-x-4" style="background-color:transparent;">
						<li>
							<a href="{base}/" aria-current={$page.url.pathname === '/'}>
								<button
									class="border border-slate-300 bg-gray-900 hover:bg-gray-800 px-4 py-2 rounded-lg flex items-center aspect-square fill-slate-300"
								>
		...
		{#if $isMobile}
			...
			>
				<button
					class="border border-slate-300 bg-gray-900 hover:bg-gray-800 px-4 py-2 rounded-lg flex fill-slate-300 items-center aspect-square"
