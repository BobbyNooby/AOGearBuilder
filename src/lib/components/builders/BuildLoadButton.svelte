<script lang="ts">
	...
	function saveRename() {
		if (renamingBuild) {
			...
			localStorage.setItem(saveBuildType, JSON.stringify(savedBuilds));
			savedBuilds = JSON.parse(localStorage.getItem(saveBuildType)) || []; // Reload the item name
...
<div>
	...
	{#if menuIsActive}
		...
		>
			<div class="flex flex-col items-center w-full h-full mb-10">
				<button
					on:click={() => {
						menuToggle();
					}}
					class="bg-black border rounded border-white text-white font-bold text-lg w-20 h-20 p-4 relative mt-10"
				>
					<svg
				...
				>
					{#each filteredBuilds as savedBuild}
